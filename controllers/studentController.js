...
class StudentController {
    ...
    async uploadCV(req, res) {
        ...
        try {
            ...
            // DO NOT save parsed data to DB yet. Only hold it in memory and send to client.
            const student = req.student;

            // Save only the CV file info, not personal data
            student.cvFile = {
        ...
        try {
            ...
            if (!forceNew) {
                ...
                if (recentSession) {
                    ...
                    if (!recentSession.sessionId) {
                        recentSession.sessionId = uuidv4();
    ...
    async saveProfile(req, res) {
        try {
            ...
            if (!profileData || typeof profileData !== 'object') {
                return res.status(400).json({ success: false, error: 'Invalid profile data format.' });
            }

            const student = await studentService.updateProfileFromChat(sessionId, profileData);
            ...
            try {
                ...
                console.error(`Failed to send confirmation email to ${student.email}:`, emailError);
            }
            ...
            const errorResponse = StudentUtils.formatErrorResponse(error.message, 500);
            res.status(errorResponse.statusCode).json(errorResponse.response);
        }
    }
